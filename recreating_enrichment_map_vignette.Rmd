...
selectAllNodes(EM_ex_6) ## with wordcloud when I run this I get a window asking to cluster. 
getSelectedNodes(EM_ex_6)
getWindowList(cy)

##GET http://example.com/v1/networks/networkId/nodes/selected/
```

```{r, cache = TRUE}
connect_to_R_session <- existing.CytoscapeWindow("EM7_Enrichment Map",
                                                                      copy.graph.from.cytoscape.to.R = TRUE)
## get error "ERROR in RCy3::getGraphFromCyWindow(): Node names cannot contain parentheses."

selectAllNodes(connect_to_R_session)
graph_selected_nodes <- getSelectedNodes(connect_to_R_session)
...
autoa_params <- list(adjacentWordBonus = 8,   # int
                     labelColumn = "EM10_GS_DESCR", # string
                     maxWords = 30,        # int
                     nodeList = "all"
...
EM_ex_7 <- setAutoAnnotateProperties(connect_to_R_session,
                                      "label-clusterBoosted",
                                      autoa_params)

request.uri <- "http://localhost:1234/v1/commands/autoannotate/label-clusterBoosted"
request.res <- GET(url = request.uri,
                                 query = autoa_params)
## http://localhost:1234/v1/commands/autoannotate/label-clusterBoosted?adjacentWordBonus=8&labelColumn=EM10_GS_DESCR&maxWords=30&nodeList=all
request.res
## same problem as in clustermaker...
...
wordcloud create Creates a Word Cloud from a list of nodes. This is an ObservableTask that returns a result. Result type: Map.class. The map contains 4 parallel lists. Key: "name", Value: String, Cloud name. Key: "size", Value: Integer, Number of words in the cloud (and size of each List). Key: "words", Value: List<String>, List of words in the cloud. Key: "fontSizes", Value: List<Integer>, The font size of each word. Key: "clusters", Value: List<Integer>, An ID for the cluster that the word belongs to. Key: "numbers", Value: List<Integer>, A unique ID for each word. 

wordcloud delete Deletes a cloud
wordcloud select Selects the nodes that are associated with the cloud
wordcloud version Return wordcloud app version. This is an ObservableTask that returns a result. Result type: int[].class. The returned array contains 3 elements: major version, minor version, patch version

wordcloud create arguments:
cloudGroupTableName=<String>: Cloud group table name (deprecated)
...
wordcloud delete arguments:
cloudName=<String>: Name of cloud to be destroyed

wordcloud select arguments:
cloudName=<String>: Name of cloud to be selected
updateNodeSelection=true|false: If true then the graph nodes will be selected to match the cloud. Default: true


```{r, cache = TRUE}
## get version
request.uri <- "http://localhost:1234/v1/commands/wordcloud/version"
request.res <- GET(url = request.uri)
request.res
rawToChar(request.res$content)
...
## would like an export function like in the GUI.
